...
%     subplot(8,19,k)
    ...
    set(gca,'XScale','log')
    lim = get(gca,'xlim');
    temp = [data(k).background.rawmean.translation.fw.w2 data(k).background.rawmean.translation.fw.w4 data(k).background.rawmean.translation.fw.w8...
        data(k).background.rawmean.translation.bw.w2 data(k).background.rawmean.translation.bw.w4 data(k).background.rawmean.translation.bw.w8...
        data(k).background.rawmean.rotation.cw.w2 data(k).background.rawmean.rotation.cw.w4 data(k).background.rawmean.rotation.cw.w8...
        data(k).background.rawmean.rotation.ccw.w2 data(k).background.rawmean.rotation.ccw.w4 data(k).background.rawmean.rotation.ccw.w8];
    ...
    plot(data(k).xfreq.w2, data(k).yfreq.mean.(char(type)).(char(dir)).w2,'-', 'Color', col, 'LineWidth', 1)
    plot(data(k).xfreq.w4, data(k).yfreq.mean.(char(type)).(char(dir)).w4,'-', 'Color', col, 'LineWidth', 2)
    plot(data(k).xfreq.w8, data(k).yfreq.mean.(char(type)).(char(dir)).w8,'-', 'Color', col, 'LineWidth', 3)
    set(gca,'Box','on')
